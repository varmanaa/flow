...
export async function handle_add_name_command(
  context: Context,
  ...
  if (is_lower_name_value_used) {
    embed.description = `**${name_value}** was already added, but with a different spelling. Provide a different name.`
  } else {
    ...
    context.cache.insert_identifier(event.id, lower_name_value)

    embed.description = `I've added **${name_value}** to the "${event.label}" attendee list.`
  }
